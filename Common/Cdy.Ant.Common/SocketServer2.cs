        ...
        public virtual void Start(int port)
        {
            if (!isRunning)
            {
                ...
                catch (System.Net.Sockets.SocketException se)
                {
                    if (se.SocketErrorCode == System.Net.Sockets.SocketError.AccessDenied)
                    {
                        Console.WriteLine("Server " + "启动服务失败,端口 " + port + " 被占用", ConsoleColor.Red);
                    }
                    else
                    {
                        Console.WriteLine("Server " + "在端口 " + port  + " 启动服务失败, ", ConsoleColor.Red);
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine("Server " + "在端口 " + port + " 启动服务失败, " + ex.Message + ex.StackTrace, ConsoleColor.Red);
                }
            }
        }
